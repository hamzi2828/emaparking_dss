...
class Booking extends BaseModel
{
    ...
    {
        if($this->isInCancellation()) {
            return 'Pending cancellation';
        }
        if($this->isInAmendment()) {
    ...
    {
        return $this->hasOne(\Modules\User\Models\User::class, 'id', 'customer_id');
    }
    ...
    {
        return $this->hasMany(BookingUpdateReason::class, 'booking_id', 'id');
    }
    ...
    public function sendNewBookingEmails()
    {
        try {
            ...
            Mail::to('ema_bookings@emaparking.co.uk')->send(new NewBookingEmail($this, 'admin'));
            // to Vendor
            //Mail::to(User::find($this->vendor_id))->send(new NewBookingEmail($this, 'vendor'));

            // To Customer
            SendUnpaidEmail::create([
                'email'=>$this->email,
                'booking_id' => $this->id,
            ]);
    ...
    public function sendStatusUpdatedEmails(){
        ...
        try{
            ...
            if($this->status=='paid') {
                Mail::to('ema_bookings@emaparking.co.uk')->send(new StatusUpdatedEmail($this,'admin'));
            }
    ...
    {
        return $this->hasMany(BookingCancellation::class, "booking_id", 'id');
    }

    public function amendment_requests()
    {
        return $this->hasMany(BookingAmendment::class, "booking_id", 'id');
    }

    public function isInCancellation() {
        return $this->cancellation_requests()->where('status','pending')->count()>0;
    }

    public function isInAmendment() {
        return $this->amendment_requests()->where('status','pending')->count()>0;
    }

    public function alreadyCancelled() {
        return $this->cancellation_requests()->where('status','approved')->count()>0;
    }

    public function cancellation_status() {
        return $this->cancellation_requests()->orderBy('id','DESC')->first()->status;
    }

    public function amendment_status() {
        return $this->amendment_requests()->orderBy('id','DESC')->first()->status;
    }
    ...
    public static function getBookingHistory($booking_status = false, $customer_id = false , $vendor_id = false , $service = false, $perpage = 10)
    {
        ...
        return $list_booking->paginate($perpage);
    }
