...
@section ('content')
    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{__("Availability & Pricing Calendar")}}</h1>
        </div>
...
<!--            <div class="panel-title"><strong>{{__('Availability')}}</strong></div>-->
            <div class="panel-body no-padding" style="background: #f4f6f8;padding: 0px 15px;">
                <div class="row">
                    <div class="col-md-3 d-none" style="border-right: 1px solid #dee2e6;">
                        <ul class="nav nav-tabs  flex-column vertical-nav" id="items_tab"  role="tablist">
                            @foreach($rows as $k=>$item)
                                <li class="nav-item event-name" style="margin-right: 0px; white-space: inherit;">
        ...
        @else
            <div class="alert alert-warning">{{__("No parking spaces found")}}</div>
        @endif
                ...
                <div class="modal-body">
                    <form class="row form_modal_calendar form-horizontal" novalidate onsubmit="return false">
                        ...
                        <div class="col-md-6" v-show="form.active">
                            <div class="form-group">
                                <label >{{__('Pricing plan')}}</label>
                                <select v-model="form.plan_id" class="form-control">
                                @foreach($plans as $plan)
                                    <option value="{{$plan->id}}">{{$plan->name}}</option>
                                @endforeach
                                </select>
&lt;!&ndash;                                <input type="number"  v-model="form.price" class="form-control">&ndash;&gt;
                            </div>
                        </div>
    ...
    <script>
        ...
        $('#items_tab').on('show.bs.tab',function (e) {
			...
			calendar = new FullCalendar.Calendar(calendarEl, {
				...
				events:{
                    	url:"{{route('space.admin.availability.loadDates-count')}}",
						extraParams:{
							id:lastId,
                        }
                },
