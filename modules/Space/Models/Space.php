...
class Space extends Bookable
{
    ...
    public $checkout_booking_detail_review_modal_file = 'Space::frontend/booking/review-modal';
    public $checkout_booking_detail_cancel_modal_file = 'Space::frontend/booking/cancel-modal';

    public $checkout_booking_detail_amendment_modal_file = 'Space::frontend/booking/amendment-modal';
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->spacePlanClass = SpacePlans::class;
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        $param = [];
        if($include_param){
            if(!empty($date=  request()->input('date1'))){
                    $param['start'] = request()->input('date1') ?? "";
            }
            if(!empty($date =  request()->input('date2'))){
                $param['end'] = request()->input('date2') ?? "";
            }
            if(!empty($time =  request()->input('arrival_time'))){
                $param['arrival_time'] = request()->input('arrival_time') ?? "";
            }

            if(!empty($time =  request()->input('departure_time'))){
                $param['departure_time'] = request()->input('departure_time') ?? "";
            }
    ...
    public function addToCart(Request $request)
    {
        ...
        /*if ($total == 0) {
            return $this->sendError(__("Price calculation error"));
        }*/
    ...
    public function getPriceInRanges($start_date,$end_date){
        $totalPrice = 0;
        //$price = 0;
        /*$plan = SpacePlans::find($this->plan_id);
        $d = Carbon::parse($start_date)->format('d');
        $price = $plan->days[$d-1];*/
        //
        ...
        $datesRaw = $this->spaceDateClass::getStartDatesInRanges($start_date,$this->id);
        if ($datesRaw != null) {
            $plan = SpacePlans::find($datesRaw->plan_id);

            if ($days > 30) {
                $price=$plan->days[30];
                $more_days = $days - 30;
                $price += $more_days * $plan->days[31];
            }
            else {
                $price = $plan->days[$days];
            }
        }
        else {
            $plan = SpacePlans::find($this->plan_id);
       ...
       /* $datesRaw = $this->spaceDateClass::getDatesInRanges($start_date,$end_date,$this->id);
        ...
        if(!empty($datesRaw))
        {
            foreach ($datesRaw as $date){
                $plan = SpacePlans::find($date->plan_id);
                $d = Carbon::parse($date->start_date)->format('d');
                $date->price = $plan->days[$d-1];
                if($date->price != null){
    ...
    public function plan(){
        return $this->hasOne($this->spacePlanClass, 'id','plan_id');
    }
