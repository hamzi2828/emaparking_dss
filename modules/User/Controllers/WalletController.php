...
class WalletController extends FrontendController
{
    public function wallet()
    {
        ...
        $data = [
            ...
            'transactions'=>$row->transactions()->with(['payment','author'])->orderBy('id','desc')->take(5)->get()
        ];
    ...
    public function wallet_transactions()
    {
        ...
        $row = User::find(auth()->user()->id);
        $data = [
            'transactions'=>$row->transactions()->with(['payment','author'])->orderBy('id','desc')->paginate(10)
        ];
        return view('User::frontend.wallet.transactions',$data);
    }
    public function buy(){
        if( setting_item('wallet_module_disable')){
            return redirect(route("user.profile.index"));
    ...
    public function buyProcess(Request $request){
        ...
        if($success){
            $transaction = $row->depositFloat($deposit_credit,['user_deposit'=>auth()->id()],false);
