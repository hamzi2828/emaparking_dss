            ...
            @if(!empty($top_cards))
                @foreach($top_cards as $card)
                    <div class="col-sm-6 col-md-3">
                        <a href="{{$card['url']}}" class="dashboard-report-card card {{$card['class']}}">
...
<!--            <div class="col-md-12 col-lg-6 mb-3">
            ...
            <div class="col-md-12 col-lg-6 ">
                <div class="panel">
                    <div class="panel-title d-flex justify-content-between">
                        <strong>{{__('Today\'s Arrivals')}}</strong>
                        <a href="{{route('report.admin.booking').'?arrival='.$today}}" class="btn-link">{{__("More")}}
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tr>
                                    ...
                                    <th width="100px">{{__("Arrival")}}</th>
                                    <!--                                    <th width="100px">{{__("Status")}}</th>-->
                                    <!--                                    <th width="100px">{{__("Created At")}}</th>-->
                                </tr>
                                </thead>
                                <tbody>
                                @if(count($recent_arrivals) > 0)
                                    @foreach($recent_arrivals as $booking)
                                        <tr @if($booking->collection_time != null) style="background: lightgreen" @elseif($booking->status == 'no_show') style="background: palevioletred;"  @elseif($booking->status == 'cancelled') style="background: darkgrey;" @endif>
            ...
            <div class="col-md-12 col-lg-6 ">
                <div class="panel">
                    <div class="panel-title d-flex justify-content-between">
                        <strong>{{__('Today\'s Returns')}}</strong>
                        <a href="{{route('report.admin.booking').'?departure='.$today}}" class="btn-link">{{__("More")}}
                            <i class="icon ion-ios-arrow-forward"></i></a>
                    </div>
                    <div class="panel-body">
                        <div class="table-responsive">
...
<!--                                    <th>{{__('Item')}}</th>-->
                                    <th width="100px">{{__("Customer")}}</th>
                                    <!--                                    <th width="100px">{{__("Paid")}}</th>-->
                                    <th width="100px">{{__("Return")}}</th>
<!--                                    <th width="100px">{{__("Status")}}</th>-->
<!--                                    <th width="100px">{{__("Created At")}}</th>-->
                                </tr>
                                </thead>
                                <tbody>
                                @if(count($recent_pickups) > 0)
                                    @foreach($recent_pickups as $booking)
                                        <tr @if($booking->return_time != null) style="background: lightgreen" @elseif($booking->status == 'no_show') style="background: palevioletred;" @elseif($booking->status == 'cancelled') style="background: darkgrey;" @endif>
                                            <td>#{{$booking->reference_no}}</td>
<!--                                            <td>
                                                @if(get_bookable_service_by_id($booking->object_model) and $service = $booking->service)
                                                    <a href="{{$service->getDetailUrl()}}" target="_blank">{{$service->title}}</a>
