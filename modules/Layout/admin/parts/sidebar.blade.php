...
<ul class="main-menu pb-5">
    @foreach($menus as $menuItem)
        {{--@if(clean($menuItem['title'],['Attr.AllowedClasses'=>null]) == 'User Plans')
            @continue;
        @endif--}}
        @if(in_array($menuItem['title'], ['Themes', 'Payouts', 'Tools']))
            @continue
        @endif
        ...
        <li class="{{$menuItem['class']}}"><a href="{{ url($menuItem['url']) }}">
                ...
                @endif
                @if(clean($menuItem['title'],['Attr.AllowedClasses'=>null]) == 'Location')
                    Companies
                @elseif(clean($menuItem['title'],['Attr.AllowedClasses'=>null]) == 'Space')
                    Parking Spaces
                @else
            ...
            @if(!empty($menuItem['children']))
                ...
                <ul class="children">
                    @foreach($menuItem['children'] as $menuItem2)
                        @if(in_array($menuItem2['title'], ['Hotel Settings', 'Car Settings', 'Event Settings', 'Tour Settings', 'Flight Settings', 'Boat Settings', 'Mobile App Settings']))
                            @continue
                        @endif
                        <li class="{{$menuItem['class']}}">
                            <a href="{{ url($menuItem2['url']) }}">
                                ...
                                @endif
                                @if($menuItem['title'] == 'Location' && $menuItem2['title'] == 'All Category')
                                    Nearby attributes
                                @else
