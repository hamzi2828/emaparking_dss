    ...
    class Bookable extends BaseModel
    {
        ...
        public function getDisplayPriceAttribute()
        {
            ...
            /*if (!empty($this->price) and $this->price > 0 and !empty($this->sale_price) and $this->sale_price > 0 and $this->price > $this->sale_price) {
                return format_money(10);
            }
            return format_money(10);*/
            $plan = SpacePlans::find($this->plan_id);
            $price =  $plan ? $plan->days[0] : 0;
            return format_money($price);
        }

        public function price_range_calculate($start = null, $end = null,$coupon = null) {
            if($start == null) {
                $start = display_date(strtotime("+1 day"));
            }

            if ($end == null) {
                $end = display_date(strtotime("+7 day"));
            }
            ...
            //$space_date = SpaceDate::where('target_id', $this->id)->whereDate('start_date', '<=',$start)->whereDate('end_date', '>=',$start)->first();

            $days = $end->diffInDays($start);

            //

            $datesRaw = $this->spaceDateClass::getStartDatesInRanges($start,$this->id);
            if ($datesRaw != null) {
            ...
            if ($coupon!=null) {
                $coupon = Coupon::where('code',$coupon)->where('is_partner',0)->where('status','publish')->first();
                $check = true;
                if ($coupon) {
                    if(!empty($end_date = $coupon->end_date)){
                ...
                $coupon = Coupon::where('auto_apply',true)->where('is_partner',0)->where('status','publish')->where(function ($query) {
            ...
            /*if ($space_date != null) {
                $plan = SpacePlans::find($space_date->plan_id);
            }
            else {
                $plan = SpacePlans::find($this->plan_id);
            }
        ...
        public function price_range_calculate_before_discount($start = null, $end = null, $coupon = null) {
            if($start == null) {
                $start = display_date(strtotime("+1 day"));
            }
        ...
        public function total_days_calculate($start = null, $end = null) {
            if($start == null) {
                $start = display_date(strtotime("+1 day"));
            }
