@if(!empty($list_term))
    <div class="bravo-featured-box bg-gray-2">
        <div class="container space-1">
            ...
            <div class="row mb-2">
                <?php
                    $urls = ['https://emaparking.co.uk/space/midlands-meet-greet-flexible','https://emaparking.co.uk/space/midlands-meet-greet-non-flexible', 'https://emaparking.co.uk/space/midlands-meet-greet-undercover']
                ?>
                @foreach($list_term as $key=> $item)
                    <?php
                        $image_url = get_file_url($item->image_id, 'full');
                        //$page_search = Modules\Space\Models\Space::getLinkForPageSearch(false , [ 'terms[]' =>  $item->id] );
                        $page_search = $urls[$key];
                    ?>
                    <div class="col-md-6 col-xl-3 mx-auto">
                        <div class="rounded-xs mb-5">
                            <a class="d-block" href="{{ $page_search }}">
                                <div class="bg-img-hero gradient-overlay-half-bg-white transition-3d-hover shadow-hover-2 max-height-200" style="background-image: url({{asset('/uploads/0000/1/2023/03/29/parking.jpg')}});">
                                    <div class="text-center py-5">
                                        <i class="{{$item->icon}} font-size-50"></i>
