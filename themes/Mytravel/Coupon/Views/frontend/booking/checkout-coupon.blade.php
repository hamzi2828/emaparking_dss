<li class="d-flex flex-column border-0 mb-0 mt-2">
    <div class="section-coupon-form">
        @if(in_array($booking->status , ['draft']))
            @if($booking->coupons->count()==0)
            <div class="input-group group-form mb-2">
                <input type="text" class="form-control form-control-sm" name="coupon_code" placeholder="{{ __("Coupon code") }}" value="{{session()->get('coupon')}}">
        ...
        @if(!empty($booking->coupons))
            <div class="font-size-16 font-weight-bold">Coupon:</div>
